// Guards the production bundle (run after `npm run build`): nothing from the dev
// tooling (lab/*.html, src/dev/**, scripts/**) may end up in dist/, and the analysis
// worker must be emitted as its own chunk referenced relatively.
...
for (const f of files.filter((f) => f.endsWith('.js.map'))) {
  ...
  for (const s of map.sources) if (/(^|\/)(src\/dev|lab|scripts)\//.test(s)) problems.push(`${f} bundles dev source ${s}`)
}
for (const f of files.filter((f) => f.endsWith('.js'))) {
  const js = readFileSync(join(DIST, f), 'utf8')
  for (const marker of ['__revealLab', '__useGame', 'FX_NOW', '/lab/']) if (js.includes(marker)) problems.push(`${f} contains dev marker "${marker}"`)
}
...
console.log(problems.length ? problems.join('\n') : 'bundle OK: no lab/ or src/dev/ code, relative URLs, separate worker chunk')
