// Fix-net lab (copy of the net lab + extra probes): drives the real transport
// against the public PeerJS server. Exposes window.netLab for scripts/fix-net/*.mjs.
...
/** A large, realistic-looking state (kept local: fixtures.ts is edited by other teams). */
function bigState(): HostMsg {
  const tracks = Array.from({ length: 14 }, (_, i) => ({
    ...
    artist: 'Artista',
    album: 'Album',
    cover: `https://cdn-images.dzcdn.net/images/cover/${'a'.repeat(32)}/1000x1000-000000-80-0-0.jpg`,
  ...
  const row = Array.from({ length: 8 }, (_, p) => ({ playerId: `p${p}`, order: [3, 1, 4, 0, 5, 2, 7, 6], points: 1234, correct: 3, pairs: 2, timedOut: false, at: 1 }))
  const state = { code: 'KXQPM', hostId: 'host', seq: 99, players: [], settings: {}, phase: { kind: 'final' }, tracks, rounds: [], submissions: {}, ready: {}, results: Array.from({ length: 10 }, () => [...row, ...row]) }
  return { t: 'state', hostNow: Date.now(), state: state as unknown as Extract<HostMsg, { t: 'state' }>['state'] }
}
...
const lab = {
  ...
  iceServers: (): RTCIceServer[] => netDebug.iceServers(),
  prepareIce: (waitMs?: number): Promise<void> => netDebug.prepareIce(waitMs),
  setTurnUrl: (url: string | null): void => netDebug.turnCredentialsUrl(url),
  setSignaling: (o: Parameters<typeof netDebug.signalingServer>[0]): void => netDebug.signalingServer(o),
  signaling: () => netDebug.signaling(),
  /** Fires a synthetic pagehide / pageshow on this window (what a tab close / bfcache does). */
  pageHide: (): void => void window.dispatchEvent(new PageTransitionEvent('pagehide', { persisted: false })),
  pageShow: (persisted: boolean): void => void window.dispatchEvent(new PageTransitionEvent('pageshow', { persisted })),
  /** Fires 'online' on window (network came back). */
  online: (): void => void window.dispatchEvent(new Event('online')),
  /** ICE connection states of every live RTCPeerConnection. */
  iceStates: (): string[] => pcs.map((pc) => `${pc.iceConnectionState}/${pc.connectionState}`),
  /** How many RTCPeerConnections were constructed so far (validation probes included). */
  pcCount: (): number => pcs.length,

  /**
   * Server behaviour probe: from one fresh Peer, send `n` offers to an unknown id,
   * `spacingMs` apart; report for each whether/when 'peer-unavailable' came back.
   */
  async probeUnknown(n: number, spacingMs: number, sameTarget = true) {
    const { Peer } = await import('peerjs')
    const rnd = () => Math.random().toString(36).slice(2, 10)
    const p = new Peer('unshuffle-v1-probe-' + rnd())
    ...
    for (let i = 0; i < n; i++) {
      const dst = sameTarget ? target : 'unshuffle-v1-NOPE' + rnd()
      const t0 = performance.now()
      const got = await new Promise<string>((resolve) => {
        const onErr = (e: { type: string; message: string }) => {
          if (e.type === 'peer-unavailable' && e.message.includes(dst)) {
            p.off('error', onErr)
            resolve(`PU ${Math.round(performance.now() - t0)}ms`)
          }
        }
        p.on('error', onErr)
        const c = p.connect(dst, { serialization: 'json', reliable: true })
        setTimeout(() => {
          p.off('error', onErr)
          try {
            c.close()
  ...
  /** Alternating sources → one unknown target: is the "every other offer" dedupe per target or per source? */
  async probeSources(rounds: number, spacingMs: number) {
    const { Peer } = await import('peerjs')
    const rnd = () => Math.random().toString(36).slice(2, 10)
    const open = async () => {
      const p = new Peer('unshuffle-v1-probe-' + rnd())
    ...
    const b = await open()
    const target = 'unshuffle-v1-NOPE' + rnd()
    const offer = (p: InstanceType<typeof Peer>, dst: string) =>
    ...
    for (let i = 0; i < rounds; i++) {
      out.push(`A: ${await offer(a, target)}`)
      await new Promise((r) => setTimeout(r, spacingMs))
      out.push(`B: ${await offer(b, target)}`)
      await new Promise((r) => setTimeout(r, spacingMs))
    }
    a.destroy()
    b.destroy()
  ...
  /** A raw OFFER message (no RTCPeerConnection) to a fresh random id: EXPIRE back? */
  async probeRawOffer(n: number) {
    const { Peer } = await import('peerjs')
    const rnd = () => Math.random().toString(36).slice(2, 10)
    const p = new Peer('unshuffle-v1-probe-' + rnd())
    await new Promise<void>((r, j) => {
    ...
    const out: string[] = []
    for (let i = 0; i < n; i++) {
      const dst = 'unshuffle-v1-probe-x' + rnd()
      const t0 = performance.now()
      const got = await new Promise<string>((resolve) => {
        const onErr = (e: { type: string; message: string }) => {
          if (e.type === 'peer-unavailable' && e.message.includes(dst)) {
            p.off('error', onErr)
            resolve(`EXPIRE ${Math.round(performance.now() - t0)}ms`)
          }
        }
        p.on('error', onErr)
        p.socket.send({ type: 'OFFER', payload: { sdp: { type: 'offer', sdp: '' }, type: 'data', connectionId: 'dc_probe_' + rnd(), serialization: 'json', reliable: true, label: 'probe' }, dst })
        setTimeout(() => {
          p.off('error', onErr)
          resolve('none in 5s')
        }, 5000)
  ...
  /** Does a bare signaling CANDIDATE to an unknown id come back as EXPIRE (peer-unavailable)? */
  async probeCandidate(n: number) {
    const { Peer } = await import('peerjs')
    const rnd = () => Math.random().toString(36).slice(2, 10)
    const p = new Peer('unshuffle-v1-probe-' + rnd())
    await new Promise<void>((r, j) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const got = await new Promise<string>((resolve) => {
        ...
        p.on('error', onErr)
        p.socket.send({ type: 'CANDIDATE', payload: { candidate: null, type: 'data', connectionId: 'dc_probe' }, dst })
        setTimeout(() => {
          p.off('error', onErr)
          resolve('none in 6s')
        }, 6000)
...
// Same global name as the net lab (the scripts drive both), but no `declare global`:
// two augmentations of Window.netLab with different shapes would clash.
;(window as unknown as { netLab: typeof lab }).netLab = lab
...
app.innerHTML = `
  <div style="padding:16px;max-width:980px;margin:0 auto">
    <h1 style="font:800 18px system-ui;letter-spacing:.08em;margin:0 0 12px">UNSHUFFLE · FIX-NET LAB</h1>
...
app.addEventListener('click', (e) => {
  ...
  if (a === 'ping') {
    if (client) lab.clientSend({ t: 'ping', c: Date.now() })
    else lab.hostBroadcast({ t: 'pong', c: 0, h: Date.now() })
  }
