...
import { loadPreview } from '../round/deezer'
...
// ---- fix-round extras: players=N, me=<id>, name2=<first confirmer's name>, host=0 (view as guest)
const EXTRA_NAMES = ['Signor Maritozzo', 'Lady Cannoli', 'Capitan Arancino', 'DJ Pinguino Supremo', 'Nonna Rock', 'Zio Bassline', 'Mister Tiramisù', 'La Vespa']
function withParams(sc: Scene): Scene {
  let room = sc.room
  const n = Number(params.get('players') ?? 0)
  if (n > 0) {
    const base = room.players.filter((p) => p.activeFromRound <= 2)
    const extra = Array.from({ length: Math.max(0, n - base.length) }, (_, i) => ({
      id: `p-x${i}`,
    ...
    room = { ...room, players: [...base, ...extra].slice(0, n) }
  }
  const name2 = params.get('name2')
  if (name2) room = { ...room, players: room.players.map((p) => (p.id === 'p-2' ? { ...p, name: name2 } : p)) }
  if (params.get('tie') === '1') {
    // Tommy and Giulia on equal points; Giulia confirmed faster overall → Tommy is 2nd.
    room = { ...room, players: room.players.map((p) => (p.id === 'p-2' ? { ...p, score: 13840 } : p)), results: [[{ playerId: 'p-2', order: [], correct: 0, pairs: 0, points: 0, perfect: false, timeMs: 30000, timedOut: false }, { playerId: 'p-host', order: [], correct: 0, pairs: 0, points: 0, perfect: false, timeMs: 50000, timedOut: false }]] as RoomState['results'] }
  }
  return { ...sc, room, me: params.get('me') ?? sc.me }
}

function scene(name: string, audioStatus: AudioStatus): Scene {
  return withParams(sceneBase(name, audioStatus))
}

function sceneBase(name: string, audioStatus: AudioStatus): Scene {
    ...
    <MotionConfig reducedMotion="user">
      <BoardAudioProvider engine={engine} sfx={sfx}>
        ...
        <RoundView
          ...
          onLeave={() => labEvents.push('leave')}
          onEndGame={() => labEvents.push('endGame')}
        />
