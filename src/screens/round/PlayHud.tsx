...
function WideHud({ info, remainingMs, totalMs, running, finalMode, players, checked, me, score, rank, showPlayers, banner = null }: PlayHudProps) {
  ...
  return (
    ...
    >
      <div className="relative flex min-w-0 items-center gap-2 lg:gap-3">
        ...
        <motion.div className="min-w-0" animate={{ opacity: bannerOn ? 0 : 1, scale: bannerOn && !reduce ? 0.96 : 1 }} transition={{ duration: 0.2 }}>
          <HudPanel side="left" className={panel}>
            ...
            <Divider />
            <Cell label={t('round.hud.snippets')}>
      ...
      >
        <TimerRing remainingMs={remainingMs} totalMs={totalMs} running={running} size={lg ? 112 : 100} caption={finalMode ? t('round.hud.finalTime') : t('round.hud.time')} />
        {finalMode && (
          <motion.span
      ...
      <div className="flex min-w-0 items-center justify-end gap-3">
        <HudPanel side="right" className={panel}>
          {showPlayers && (
            <>
              <Cell label={t('round.hud.confirmed', { done, total: players.length })} className="shrink">
                <PlayersStrip players={players} checked={checked} me={me} size="sm" max={xl ? 7 : lg ? 6 : 4} overlap={!xl} label={t('round.hud.players')} />
              </Cell>
              <Divider />
            </>
          )}
          <Cell label={t('round.hud.points')} align="end" className="shrink-0">
            <AnimatedNumber value={score} className={cn('block leading-none font-bold text-gold', big)} />
            {rank > 0 && players.length > 1 && <span className="text-[12px] font-bold leading-none whitespace-nowrap text-ink-300">{t('round.hud.rank', { rank: formatOrdinal(rank) })}</span>}
          </Cell>
        </HudPanel>
      ...
      <div className="glass-flat relative rounded-[22px] px-3 pt-2.5 pb-3">
        <div className="flex h-10 items-center gap-2.5">
          ...
          <div className="flex shrink-0 flex-col gap-1">
            <span className="eyebrow text-[9px] leading-none">{t('round.hud.round')}</span>
          ...
          {showPlayers && (
            <PlayersStrip players={players} checked={checked} me={me} size="xs" max={landscape ? 9 : narrow ? 5 : 6} overlap className="ml-auto min-w-0" label={t('round.hud.players')} />
          )}
          <div className={cn('flex shrink-0 flex-col items-end gap-1', !showPlayers && 'ml-auto')}>
            <span className="eyebrow text-[9px] leading-none">{t('round.hud.points')}</span>
        ...
        <div className="mt-2.5 flex items-center gap-2">
          {finalMode && (
            <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="grid size-5 shrink-0 place-items-center rounded-full bg-coral text-white">
              <Icon name="bolt" size={12} filled strokeWidth={2.4} label={t('round.hud.lastSeconds')} />
            </motion.span>
          )}
