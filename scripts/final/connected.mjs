...
for (const me of ['p-host', 'p-3']) {
  ...
  await p.goto(`http://localhost:5214/lab/final.html?connected=1&ui=0&me=${me}`, { waitUntil: 'networkidle' })
  ...
  if (me === 'p-host') await p.screenshot({ path: new URL('./shots/connected-phone.png', import.meta.url).pathname })
  await ctx.close()
}
await browser.close()
