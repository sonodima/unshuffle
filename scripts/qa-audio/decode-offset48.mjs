...
const tracks = JSON.parse(readFileSync(new URL('./tracks.json', import.meta.url), 'utf8')).slice(3, 6)
const urls = []
...
async function decodeIn(bt, opts) {
  ...
  const out = await p.evaluate(async (urls) => {
    ...
    for (const u of urls) {
      ...
      res.push({ label: u.label, length: buf.length, sr: buf.sampleRate, head: Array.from(d.subarray(0, 8000)), win: Array.from(d.subarray(480000, 480000 + 4096)), mid: Array.from(d.subarray(470000, 500000)) })
    }
    return res
  }, urls)
  await b.close()
...
for (let i = 0; i < C.length; i++) {
  const c = C[i], w = W[i]
  // Find lag of chrome's 4096-sample window (at 480000) inside webkit's 30000-sample block starting at 470000.
  ...
  const offset = 470000 + lag - 480000 // webkit index - chrome index for the same audio
  const firstNz = (a) => a.findIndex((v) => Math.abs(v) > 1e-4)
  console.log(c.label.padEnd(36), `chrome len ${c.length} webkit len ${w.length} (Δ ${w.length - c.length})`, `offset webkit-chrome ${offset} samples = ${(offset / 48).toFixed(2)} ms (corr ${best.toFixed(3)})`, `first>1e-4 chrome ${firstNz(c.head)} webkit ${firstNz(w.head)}`)
}
