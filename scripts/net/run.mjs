// End-to-end validation of src/net/transport.ts against the REAL public PeerJS
// server. One Chrome, separate contexts for host / clients.
//   UNSHUFFLE_VITE_CACHE=node_modules/.vite-net npx vite --port 5205 --strictPort &
//   node scripts/net/run.mjs [--skip-slow]
import { chromium } from 'playwright'

const URL = process.env.NET_LAB_URL ?? 'http://localhost:5205/lab/net.html'
...
async function newPage(label) {
  ...
  page.on('console', (m) => {
    if (m.type() === 'error' || m.type() === 'warning') console.log(`[${label}] ${m.type()}: ${m.text().slice(0, 200)}`)
  })
  await page.goto(URL)
...
if (!SKIP_SLOW) {
  ...
  await step('host vanishes without close() → client ends with host-gone in ≤ 30 s', async () => {
    const h = await newPage('ghost-host')
    const c = await newPage('ghost-client')
    const r = await lab(h, () => window.netLab.createHost())
    const j = await lab(c, (code) => window.netLab.joinRoom(code), r.code)
    ...
    const t = Date.now()
    await h.ctx.close()
    await waitEvent(c, 'client:status', { from: c0, pred: 'd.s === "reconnecting"', timeout: 14000 })
    const tRec = Date.now() - t
    // The reconnect loop ends early once the signaling server keeps saying the code is gone
    // (NET_TIMING.hostGoneAfterMs, ~14–16 s after the tab closed).
    const closed = await waitEvent(c, 'client:status', { from: c0, pred: 'd.s === "closed"', timeout: 30000 })
    const tClosed = Date.now() - t
    const attempts = (await lab(c, () => window.netLab.log())).filter((l) => l.msg.startsWith('reconnect attempt')).length
    await sleep(1000)
    const stats = await lab(c, () => window.netLab.stats())
    await c.ctx.close()
    if (closed.data.d !== 'host-gone') throw new Error(`detail ${closed.data.d}`)
    if (tClosed > 30000) throw new Error(`closed only after ${tClosed} ms`)
    if (stats.peers || stats.timers || stats.clients) throw new Error(`leak ${JSON.stringify(stats)}`)
    return `reconnecting after ${tRec} ms, closed (host-gone) after ${tClosed} ms, ${attempts} failed attempts`
  })
}
