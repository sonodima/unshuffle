// Copied from scripts/qa-audio/drag-stall.mjs (QA finding 3), pointed at the fix-audio dev server.
...
try {
  ...
  await page.screenshot({ path: new URL(`./shots/drag-stall-x${THROTTLE}.png`, import.meta.url).pathname })
} catch (e) {
  log('FATAL', e?.stack ?? e)
} finally {
  writeFileSync(new URL(`./drag-stall-x${THROTTLE}.json`, import.meta.url), JSON.stringify(report, null, 1))
  await browser.close()
}
