...
const OFFSETS = (process.env.OFFSETS ?? '1000,600,400,250,120').split(',').map(Number)
...
try {
  ...
  for (let r = 0; r < OFFSETS.length; r++) {
    ...
    const dropAt = await g.evaluate(
      ...
      { at: localEnd - OFFSETS[r] - 3, order: target },
    )
    const t0 = dropAt
    const after = target
    ...
    const row = { round: r + 1, targetOffsetMs: OFFSETS[r], dropBeforeEndMs: Math.round(localEnd - dropAt), dragMs: dropAt - t0, before, after, scored: res.order, verdict, timedOut: res.timedOut, points: res.points }
    out.push(row)
    log(JSON.stringify(row))
    check(`9b: R${r + 1} drop ${row.dropBeforeEndMs} ms before time-up is scored`, verdict === 'COUNTED', verdict)
    await shot(g, `s9b-r${r + 1}-reveal`)
    if (r < OFFSETS.length - 1) await host.getByRole('button', { name: /Prossimo round/ }).first().click()
  }
} catch (err) {
  check('FATAL', false, err?.stack ?? String(err))
  for (const p of all) await shot(p, 's9b-FAIL')
} finally {
