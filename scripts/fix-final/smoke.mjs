...
try {
  ...
  // Suspense: during the drop the headline is the teaser; the real one lands with the winner.
  await host.waitForTimeout(700)
  const early = await host.evaluate(() => {
    const h1 = document.querySelector('[data-screen-frame]:not([inert]) #fp-title')
    return { opacity: h1 ? getComputedStyle(h1).opacity : null, teaser: !!document.querySelector('[data-screen-frame]:not([inert]) .fp-teaser') }
  })
  log('final @0.7s host:', JSON.stringify(early))
  ...
  const layout = async (p) =>
    p.evaluate(() => {
      ...
      const docks = [...f.querySelectorAll('[role="group"][aria-label="Azioni"]')].filter((e) => !e.closest('[inert]'))
      const dock = r(docks[0])
      const plates = [...f.querySelectorAll('.fp-plate')].map(r)
      const h2 = r(f.querySelector('h2'))
      const h1 = f.querySelector('#fp-title')
      return {
        vh: innerHeight,
        docks: docks.length,
        dock: dock && [Math.round(dock.top), Math.round(dock.bottom)],
        platesBottom: Math.round(Math.max(...plates.map((b) => b.bottom))),
        h2Top: h2 && Math.round(h2.top),
        h1: h1?.textContent, h1Opacity: getComputedStyle(h1).opacity,
        floating: !!document.querySelector('[data-screen-frame]:not([inert]) .fixed [role="group"][aria-label="Azioni"]'),
        soundInline: f.querySelectorAll('header button').length,
      }
    })
  const hl = await layout(host)
  const gl = await layout(guest)
  log('final layout host:', JSON.stringify(hl), '| guest:', JSON.stringify(gl))
  check(hl.docks === 1 && hl.dock[0] > hl.platesBottom, 'final (desktop): the dock sits below the podium plates')
  check(hl.h2Top >= hl.dock[1], 'final (desktop): the dock does not cover the first section heading')
  check(hl.h1Opacity === '1' && gl.h1Opacity === '1', 'final: the real headline is shown after the landing')
  // Guest replays a song of the game (fresh preview download + decode).
  const songBtn = guest.getByRole('button', { name: /^Ascolta l’anteprima di / }).first()
  await songBtn.scrollIntoViewIfNeeded()
  await songBtn.tap()
  const played = await guest
    .waitForFunction(() => document.querySelector('[data-screen-frame]:not([inert]) button[aria-pressed="true"]'), null, { timeout: 20_000 })
    .then(() => true, () => false)
  check(played, 'final: a guest can replay a song preview from "I brani della partita"')
  await guest.waitForTimeout(1500)
  await shot('final-song-playing')
  ...
  const bl = await layout(host)
  log('final bottom host:', JSON.stringify(bl))
  check(bl.floating && bl.docks === 1, 'final (desktop): scrolled past the podium, the dock floats (one copy in the a11y tree)')
  ...
  const stillPlaying = await guest.evaluate(() => !!document.querySelector('button[aria-pressed="true"]'))
  log('guest preview button still pressed in lobby:', stillPlaying)
  await host.waitForTimeout(900)
