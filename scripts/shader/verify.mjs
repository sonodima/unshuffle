...
// 7. WebGL1 path (WebGL2 unavailable).
{
  ...
  await page.screenshot({ path: new URL('./shots/verify-webgl1.png', import.meta.url).pathname })
  await ctx.close()
}
...
// 8. No WebGL at all → CSS gradient fallback.
{
  ...
  await page.screenshot({ path: new URL('./shots/verify-css.png', import.meta.url).pathname })
  await ctx.close()
}
