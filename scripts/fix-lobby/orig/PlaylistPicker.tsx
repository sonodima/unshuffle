...
export function PlaylistCard({ playlist, selected, onSelect }: { playlist: PlaylistRef; selected: boolean; onSelect(): void }) {
  return (
    ...
    >
      <span className="relative block">
        ...
        {!selected && (
          <span className="pointer-events-none absolute inset-x-0 bottom-2 flex justify-center opacity-0 transition-[opacity,transform] duration-200 group-hover:translate-y-0 group-hover:opacity-100 translate-y-1 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
            <span className="btn-label rounded-full bg-ink-950/80 px-3 py-1.5 font-display text-[10px] font-bold tracking-wider text-white uppercase backdrop-blur-md">
              Scegli
            </span>
          </span>
        )}
      ...
      <span className="mt-0.5 truncate px-0.5 text-[11px] font-semibold text-ink-400 @md:text-xs">
        <span className="num">{formatNumber(playlist.nbTracks)}</span> brani{playlist.creator ? ` · ${playlist.creator}` : ''}
      </span>
    </button>
  )
}
