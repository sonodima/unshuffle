...
import { Button, Icon, IconButton, Modal, Panel, cn, playSfx, useCanHover, useMediaQuery } from '../../components/ui'
import { QrCode } from './QrCode'
import { canNativeShare, nativeShare, prettyUrl, useCopy } from './invite'
...
export function RoomCodeCard({ code, joinUrl, variant = 'full', className }: RoomCodeCardProps) {
  ...
  // Very narrow phones: drop the button icons so both labels fit.
  const narrow = useMediaQuery('(max-width: 379px)')
  ...
  return (
    <Panel as="section" aria-label="Codice stanza" padding={compact ? 'sm' : 'lg'} glow="magenta" className={cn(compact && 'px-4 pt-3.5 pb-4', className)}>
      ...
      {compact ? (
        <div className="mt-4 flex gap-2 sm:justify-center">
          <Button
            variant="glass"
            size="md"
            leftIcon={narrow ? undefined : link.state === 'copied' ? 'check' : 'link'}
            className={cn('min-w-0 flex-1 max-[379px]:px-3 sm:max-w-[220px]', link.state === 'copied' && 'text-lime')}
            onClick={copyLink}
            sound={false}
          >
          ...
          {shareable && (
            <Button variant="glass" size="md" leftIcon={narrow ? undefined : 'share'} className="min-w-0 flex-1 max-[379px]:px-3 sm:max-w-[220px]" onClick={share}>
              Condividi
            </Button>
          )}
      ...
      >
        <div className="flex flex-col items-center gap-5">
          ...
          <div className="flex w-full items-center gap-2 rounded-2xl border border-white/[0.08] bg-ink-950/50 py-1.5 pr-1.5 pl-4 shadow-well">
            <span className="num min-w-0 flex-1 truncate text-[13px] text-ink-200 select-all" title={joinUrl}>
              {prettyUrl(joinUrl)}
            </span>
            <Button
...
const TILE = {
  md: 'h-[58px] max-w-[60px] rounded-[14px] text-[32px] sm:h-[76px] sm:max-w-[68px] sm:rounded-[18px] sm:text-[40px]',
