...
export function PlaylistHero({ playlist, isHost, onChange, layout = 'row', size, className }: PlaylistHeroProps) {
  ...
  return (
    ...
    >
      <Record playlist={playlist} sleeve={sleeve} />
      <div className={cn('min-w-0', column ? 'mt-7 flex w-full flex-col items-center' : 'flex-1')}>
        <span className="eyebrow">{playlist ? 'Playlist' : 'Nessuna playlist'}</span>
          ...
          >
            {playlist ? (
              <>
                ...
                <div className={cn('mt-2.5 flex flex-wrap items-center gap-x-2 gap-y-1.5', column && 'justify-center')}>
                  <Badge tone="violet" size="md" icon="music">
                    <span className="num">{formatNumber(playlist.nbTracks)}</span> brani
                  </Badge>
              ...
              <p className={cn('mt-1.5 text-sm leading-relaxed text-ink-300', column && 'max-w-[30ch]')}>
                {isHost ? 'Cerca una playlist, tocca una categoria o incolla un link Deezer.' : <WaitingText>L’host sta scegliendo la playlist</WaitingText>}
              </p>
            )}
          </motion.div>
        </AnimatePresence>
...
function Record({ playlist, sleeve }: { playlist: PlaylistRef | null; sleeve: number }) {
  const reduce = useReducedMotion()
  const disc = Math.round(sleeve * 0.94)
  const out = playlist ? Math.round(sleeve * 0.4) : Math.round(sleeve * 0.16)
  return (
    <div className="relative shrink-0" style={{ width: sleeve + out, height: sleeve }}>
      <motion.div
        className="absolute top-1/2 left-0"
        style={{ marginTop: -disc / 2, marginLeft: (sleeve - disc) / 2 }}
        initial={false}
        animate={{ x: out, rotate: 0 }}
        transition={reduce ? { duration: 0 } : { type: 'spring', stiffness: 140, damping: 18, delay: 0.12 }}
      >
        <Vinyl cover={playlist?.picture} size={disc} spin={!!playlist} period={3.2} glow={playlist ? 'var(--color-magenta)' : undefined} />
      </motion.div>
        ...
        >
          {playlist ? (
            <Cover
              ...
              className="shadow-[0_0_0_1px_rgb(255_255_255/0.12),8px_0_24px_-6px_rgb(0_0_0/0.8),0_24px_48px_-16px_rgb(0_0_0/0.9)]"
            />
          ) : (
            <div className="grid size-full place-items-center rounded-[12px] border-2 border-dashed border-white/15 bg-ink-900/70 backdrop-blur-sm">
              <Icon name="music" size={Math.round(sleeve * 0.26)} className="text-ink-500" />
            </div>
          )}
        </motion.div>
      </AnimatePresence>
